...
from app.rag.loaders import load_docx_text, load_pdf_pages
...
def _draft_chunks(
    pages: list[tuple[int | None, str]], *, title: str, chunk_size: int, chunk_overlap: int
) -> list[ChunkDraft]:
    drafts: list[ChunkDraft] = []
    for page_number, page_text in pages:
        cleaned = clean_whitespace(page_text)
        cleaned, _removed_header_chars = strip_header_block(cleaned)
        cleaned, _removed_trailer_chars = strip_trailer_section(cleaned)
        drafts.extend(
            chunk_document(
                title, cleaned, chunk_size=chunk_size, chunk_overlap=chunk_overlap, page_number=page_number
            )
        )
    return drafts
...
async def ingest_document(
    ...
    *,
    ...
    extracted_metadata = parse_header_metadata(pages[0][1]) if pages else {}

    drafts = _draft_chunks(pages, title=context.title, chunk_size=chunk_size, chunk_overlap=chunk_overlap)

    full_text = "\n".join(page_text for _, page_text in pages)
    language = detect_language(full_text, hint=language_hint or extracted_metadata.get("language"))
    extracted_metadata.setdefault("language", language)
