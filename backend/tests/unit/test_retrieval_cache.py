...
async def test_cached_retrieval_reuses_chunks_for_similar_question_embedding() -> None:
    ...
    first = await cached.retrieve("question one", [1.0, 0.0], "workspace-a", "Germany", None, 12)
    second = await cached.retrieve("similar question", [0.999, 0.001], "workspace-a", "Germany", None, 12)
...
async def test_cached_retrieval_is_scoped_by_workspace() -> None:
    ...
    await cached.retrieve("question", [1.0, 0.0], "workspace-a", None, None, 12)
    await cached.retrieve("question", [1.0, 0.0], "workspace-b", None, None, 12)
...
async def test_cached_retrieval_is_scoped_by_filters() -> None:
    ...
    await cached.retrieve("question", [1.0, 0.0], "workspace-a", "Germany", None, 12)
    await cached.retrieve("question", [1.0, 0.0], "workspace-a", "France", None, 12)
    await cached.retrieve("question", [1.0, 0.0], "workspace-a", "Germany", ["doc-1"], 12)
...
async def test_cached_retrieval_misses_when_question_embedding_is_not_similar_enough() -> None:
    ...
    await cached.retrieve("question", [1.0, 0.0], "workspace-a", None, None, 12)
    await cached.retrieve("different question", [0.0, 1.0], "workspace-a", None, None, 12)
...
class FakeRetriever:
    ...
    async def retrieve(
        ...
        del query, query_embedding, country, document_ids, candidate_count
        self.calls += 1
